    ...
    return (
        <div className='p-2 sticky top-0'>
            <h1 className='text-2xl font-bold'>Course Name</h1>
            {
                ...
                isActive ? 'bg-black text-white p-1' : undefined} to={`/courses/${info._id}`} key={info._id}>{info.title}</NavLink></div>)
            }

            <h1 className='text-2xl font-bold'>Bundle Course Name</h1>
            {
                bundlecourseName.map(binfo => <div className='pl-3 pt-2'><NavLink 
                className={({ isActive }) => isActive ? 'bg-black text-white p-1' : undefined} 
                to={`/courses/bundle/${binfo._id}`} 
                key={binfo._id}>{binfo.title}
                </NavLink></div>)
            }
